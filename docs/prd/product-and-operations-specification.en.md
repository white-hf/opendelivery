...
To streamline physical operations, **manual creation of arrival trips and handling units has been removed from this stage**. Step 4 is now a pure execution and tally workspace. Its data backbone (trip record and 10 pre-planned cages `U01~U10`) is automatically and silently generated by the system during the Step 3 Wave creation. The workspace features large, physical-status-focused state progression triggers: `EXPECTED -> ARRIVED -> UNLOADING -> READY_FOR_SCAN -> CLOSED`. Operators only start physical scanning and tally checks after confirming vehicle arrival (`ARRIVED`) and releasing scan permissions (`READY_FOR_SCAN`).
...
#### Pre-Arrival Fusion Planning Rules:
To resolve physical time-sequence inversion and supply parcel-cage mapping rules to the upstream sorting warehouse in advance, the system merges **"outbound driver assignment"** and **"inbound cage planning"** into a dual-channel unified planning workspace in Step 3.
1. **Silent Backbone Generation**: Clicking "Add Wave / Start Wave" automatically and transactionally creates a matching "Arrival Trip" and 10 pre-planned physical cages (`U01` ~ `U10`).
2. **One-click Intelligent Loading (Auto Area-Fill)**: A `⚡ Auto Preload` button runs a round-robin zoning algorithm to automatically map delivery zones (Area Versions) to these 10 cages. Operators can fine-tune this configuration via multi-select dropdowns. Mappings instantly update parcel-cage linkages, which the upstream warehouse pulls to execute bulk pre-sorting.
3. **Map-Selection Highlighting**: Selecting a cage highlights its assigned delivery areas on the planning map with a distinct purple outline and filters out irrelevant parcels, providing instantaneous and high-visibility dispatch feedback.
