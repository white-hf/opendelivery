...
Residual risk: R01 automation is complete; browser visual acceptance remains pending. Run the repeatable gate with `DB_PASSWORD=... OPS_PASSWORD=... ./scripts/delivery-area-e2e.sh`; credentials are environment-only. The map raised the Web bundle to about 1.44 MB, so the next slice must add route-level splitting. Real partner callbacks, object storage, concurrency, network partitions, and large batches remain untested.
