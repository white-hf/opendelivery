...
| R01 空间区域 | 自动化通过、运营复验中 | Flyway V8；三站共 6 个发布区域；共享边界允许，面积重叠及跨站读取拒绝；管理员三站下拉、geojson.io 混合 FeatureCollection、文件导入和绘制反馈已修复；Point 等辅助要素忽略并提示数量；区域弹窗采用并排编辑、视口限高、固定操作栏和全局通知；E2E 数据与审计已清理 |
