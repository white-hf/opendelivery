...
public class ScanSupervisionService {
    ...
    public SupervisionResponse supervision(LocalDate serviceDate, Long waveIdFilter) {
        ...
        String waveSql = """
                SELECT w.id AS wave_id, w.wave_code,
                       t.id AS task_id, t.driver_id, d.name AS driver_name
    ...
    public List<SessionSummary> sessions(Long taskId, String status, LocalDate serviceDate) {
        ...
        StringBuilder sql = new StringBuilder("""
                SELECT s.id, s.task_id, s.driver_id, d.name AS driver_name, s.status,
                       s.opened_at, s.submitted_at, s.expected_count, s.scanned_count, s.discrepancy_count,
                       COUNT(DISTINCT CASE WHEN e.result_code='EXPECTED' THEN e.tracking_no END) AS valid_cnt,
                       COUNT(CASE WHEN e.result_code='WRONG_TASK' THEN 1 END) AS wrong_cnt,
                       COUNT(CASE WHEN e.result_code='UNKNOWN' THEN 1 END) AS unk_cnt,
                       COUNT(CASE WHEN e.result_code='DUPLICATE' THEN 1 END) AS dup_cnt,
                       COUNT(CASE WHEN e.result_code='EXTRA' THEN 1 END) AS extra_cnt
                FROM scan_session s
                JOIN driver_task t ON t.id = s.task_id
                JOIN dispatch_wave w ON w.id = t.wave_id
                JOIN driver d ON d.id = s.driver_id
                LEFT JOIN scan_event e ON e.session_id = s.id
                WHERE w.station_id = ?
                """);
