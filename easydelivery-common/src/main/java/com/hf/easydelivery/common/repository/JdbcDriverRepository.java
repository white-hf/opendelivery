...
public class JdbcDriverRepository implements DriverRepository {
    ...
    public Optional<Driver> findByCredentialId(String credentialId) {
        ...
        return first(jdbc.query("""
                SELECT id, credential_id, password_hash, driver_name, status
                FROM driver WHERE credential_id = ?
                """, (rs, rowNum) -> map(rs.getLong("id"), rs.getString("credential_id"),
                rs.getString("password_hash"), rs.getString("driver_name"), rs.getString("status")), credentialId));
    }
    ...
    public Optional<Driver> findById(int id) {
        return first(jdbc.query("""
                ...
                """, (rs, rowNum) -> map(rs.getLong("id"), rs.getString("credential_id"),
                rs.getString("password_hash"), rs.getString("driver_name"), rs.getString("status")), id));
    }
    ...
    private Driver map(long id, String credentialId, String hash, String name, String status) {
        return new Driver(Math.toIntExact(id), credentialId, hash, name, status);
    }
}
