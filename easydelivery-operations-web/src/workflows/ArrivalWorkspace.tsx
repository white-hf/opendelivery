...
export function ArrivalWorkspace({session,station,serviceDate}:{session:Session;station:string;serviceDate:string}){
 ...
 return <><Tabs items={[{key:'arrival',label:t('arrival.tab'),children:arrival},{key:'manifest',label:t('arrival.legacyManifest'),children:<ManifestWorkspace session={session} station={station}/>}]} />
  <Drawer width={860} open={!!tripId} onClose={()=>setTripId(undefined)} title={`${t('arrival.detail')} · ${detail.data?.trip.external_trip_no??''}`} extra={<Button onClick={()=>setUnitOpen(true)}>{t('arrival.addUnit')}</Button>}>{detail.error&&<Alert type="error" message={detail.error.message}/>} {detail.data&&<Space direction="vertical" style={{width:'100%'}}>
   ...
   {nextTrip[detail.data.trip.status]&&<Button type="primary" onClick={()=>command.mutate({path:`/ops/v1/arrival-trips/${tripId}/state`,body:{targetStatus:nextTrip[detail.data!.trip.status],reason:'Operations arrival progression'}})}>{t('arrival.moveTo')} {nextTrip[detail.data.trip.status]}</Button>}
