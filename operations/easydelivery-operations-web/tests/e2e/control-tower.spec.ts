...
test('operator follows the business-day journey into a visible parcel map', async ({ page }) => {
    ...
    await expect(page.getByText(/Locatable \d+/)).toBeVisible();
    await expect(page.getByText(/Displayed \d+/)).toBeVisible();
    ...
    await expect(parcelMarkers.first()).toBeAttached({ timeout: 15_000 });
