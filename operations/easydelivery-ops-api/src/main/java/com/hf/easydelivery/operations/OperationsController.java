...
public class OperationsController {
    ...
    private final SystemConfigOperationsService systemConfig;
    ...
    public OperationsController(OperationsService service, RoutingOperationsService routing,
                                ...
                                ConfigCaseOperationsService configCase, SystemConfigOperationsService systemConfig) {
    ...
    public AppResponse<?> planningParcels(@RequestParam java.time.LocalDate serviceDate,
        ...
        return AppResponse.success(planning.mapParcels(serviceDate,west,south,east,north,limit,waveId,slaFilter));
    }
    ...
    @GetMapping("/system/drivers")
    public AppResponse<?> listSystemDrivers(@RequestParam(required = false) String status) {
        return AppResponse.success(systemConfig.listDrivers(status));
    }

    @PostMapping("/system/drivers")
    public AppResponse<?> createSystemDriver(@RequestBody SystemConfigOperationsService.DriverCreateRequest body) {
        return AppResponse.success("Driver created", systemConfig.createDriver(body));
    }

    @PutMapping("/system/drivers/{driverId}/status")
    public AppResponse<?> updateSystemDriverStatus(@PathVariable long driverId, @RequestBody SystemConfigOperationsService.DriverStatusRequest body) {
        systemConfig.updateDriverStatus(driverId, body);
        return AppResponse.success("Driver status updated", null);
    }

    @GetMapping("/system/service-areas")
    public AppResponse<?> listStationServiceAreas() {
        return AppResponse.success(systemConfig.listServiceAreas());
    }

    @PostMapping("/system/service-areas")
    public AppResponse<?> createStationServiceArea(@RequestBody SystemConfigOperationsService.ServiceAreaCreateRequest body) {
        return AppResponse.success("Service area created", systemConfig.createServiceArea(body));
    }

    @PostMapping("/system/stations")
    public AppResponse<?> createStation(@RequestBody SystemConfigOperationsService.StationCreateRequest body) {
        return AppResponse.success("Station created", systemConfig.createStation(body));
    }
}
